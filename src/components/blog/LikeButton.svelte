<script lang="ts">
  ...
  <div class="like-container text-pink-800">
    <button class="bg-pink-50 rounded-full w-20 h-7 p-1 active:bg-pink-200 transition-all duration-800 flex items-center" onclick={handleLike}>
      <div class="rounded-full size-6 bg-pink-200">
    ...
    {#if loading || updating}
      <p class="text-sm text-center grow"><span class="loading loading-spinner size-3"></span></p>
    {:else if error}
